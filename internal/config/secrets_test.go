...
func TestConfigRedactedAndResolvedRoundTripSecretsWithoutAliases(t *testing.T) {
	...
	current.Peer.URL = "wss://user:password@example.test/notes?token=secret"
	...
	if redacted.Mapping.Seed != RedactedValue || redacted.Peer.URL != RedactedURLValue {
		t.Fatalf("Redacted() secrets = (%q, %q)", redacted.Mapping.Seed, redacted.Peer.URL)
	}
