...
func TestClassifierCoversEveryConfigurationField(t *testing.T) {
	mutations := map[string]func(*config.Config){
		...
		"peer.token":                             func(value *config.Config) { value.Peer.Token += "changed" },
		"peer.queue_capacity":                    func(value *config.Config) { value.Peer.QueueCapacity++ },
		"peer.maximum_connections":               func(value *config.Config) { value.Peer.MaximumConnections++ },
		"peer.recent_ttl":                        func(value *config.Config) { value.Peer.RecentTTL++ },
