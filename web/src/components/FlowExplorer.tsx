          ...
          <tbody>
            {visibleFlows.map((flow) => (
              <tr key={flow.id} className={flow.soloed ? 'flow-row flow-row--soloed' : flow.muted ? 'flow-row flow-row--muted' : 'flow-row'}>
                ...
                <td><span className="musical-identity"><strong>{rootNames[flow.root] ?? `PC ${flow.root}`} {flow.mode}</strong><small>Channel {flow.channel} · {flow.fixed_identity ? 'fixed by rule' : 'automatic per flow'}</small></span></td>
                <td>{annotationsPending ? <span className="decision decision--pending">refreshing</span> : <><span className={`decision decision--${flow.state}`}>{flow.state}</span><small>{flow.rule_name || (flow.rule_id ? `${flow.rule_tier} · ${flow.rule_id}` : flow.rule_tier)}</small><details className="decision-details"><summary>Why?</summary><p>{flow.decision_reason}</p>{flow.matched_predicates.length > 0 && <ul>{flow.matched_predicates.map((predicate) => <li key={predicate}>{predicate}</li>)}</ul>}</details></>}</td>
